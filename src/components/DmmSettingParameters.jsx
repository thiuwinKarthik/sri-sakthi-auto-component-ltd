...
const DmmSettingParameters = () => {
  ...
  const [dropdowns] = useState({ operators: [], supervisors: [] });
  const [loading, setLoading] = useState(false);
  const [notification, setNotification] = useState({ show: false, type: '', message: '' });

  const createEmptyRow = (cols = columns) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const loadedMasterCols = res.data.masterCols || [];
      setColumns(loadedMasterCols);
      ...
      [1, 2, 3].forEach(shift => {
        if (res.data.shiftsData && res.data.shiftsData[shift] && res.data.shiftsData[shift].length > 0) {
          // Ensure we enforce client side unique UUIDs for rows
          loadedData[shift] = res.data.shiftsData[shift].map(row => ({ ...row, id: row.id || crypto.randomUUID() }));
        } else {
          loadedData[shift] = [createEmptyRow(loadedMasterCols)];
        }
      });
