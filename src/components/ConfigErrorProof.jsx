        ...
        <div className="min-h-screen bg-[#2d2d2d] flex flex-col font-sans relative pb-20">
            ...
            <div className="bg-[#222] border-b border-white/5 py-4 px-10 flex items-center justify-between sticky top-0 z-50">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <h1 className="text-2xl font-black text-white uppercase tracking-wider">ERROR PROOF VERIFICATION</h1>
                    </div>
                </div>
            ...
            <div className="flex-1 max-w-7xl w-full mx-auto p-10">
                ...
                <div className="bg-[#383838] border border-white/5 rounded-2xl shadow-xl overflow-hidden">
                    {/* Table Header */}
                    <div className="bg-black/20 grid grid-cols-12 gap-4 p-4 border-b border-white/5 text-[10px] font-black text-white/50 uppercase tracking-widest text-center">
                        <div className="col-span-1">S.No</div>
                        <div className="col-span-2 text-left">Line</div>
                        <div className="col-span-4 text-left">Error Proof Name</div>
                        <div className="col-span-3 text-left">Nature of Error Proof</div>
                    ...
                    {loading ? (
                        ...
                        <div className="divide-y divide-white/5">
                            {items.map((cp, index) => (
                                <div key={cp.id} className={`grid grid-cols-12 gap-4 p-4 items-start transition-all ${cp.isDeleted ? 'opacity-40 bg-red-900/10' : 'hover:bg-white/5'} ${cp.isNew ? 'border-[#ff9100]/30 border-l-4' : 'border-transparent border-l-4'}`}>

                                    <div className="col-span-1 text-center font-bold text-white/50 mt-3">
                                    ...
                                    <div className="col-span-2">
                                        <textarea
                                            ...
                                            value={cp.line}
                                            onChange={(e) => handleUpdateRow(cp.id, 'line', e.target.value)}
                                            className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-[#ff9100] focus:ring-1 focus:ring-[#ff9100] outline-none text-xs transition-all resize-none disabled:cursor-not-allowed"
                                    ...
                                    <div className="col-span-4">
                                        <textarea
                                            rows="3"
                                            disabled={cp.isDeleted}
                                            value={cp.errorProofName}
                                            onChange={(e) => handleUpdateRow(cp.id, 'errorProofName', e.target.value)}
                                            ...
                                            placeholder="Enter Error Proof Name"
                                        />
                                    </div>
                                    ...
                                    <div className="col-span-3">
                                        <textarea
                                            rows="3"
                                            disabled={cp.isDeleted}
                                            value={cp.natureOfErrorProof}
                                            onChange={(e) => handleUpdateRow(cp.id, 'natureOfErrorProof', e.target.value)}
                                            className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-[#ff9100] focus:ring-1 focus:ring-[#ff9100] outline-none text-[13px] transition-all resize-none disabled:cursor-not-allowed"
                                            placeholder="Enter Nature logic"
                                        />
                                    </div>
                                    ...
                                    <div className="col-span-1">
                                        <input
                                            ...
                                            disabled={cp.isDeleted}
                                            value={cp.frequency}
                                            onChange={(e) => handleUpdateRow(cp.id, 'frequency', e.target.value)}
                                            className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white text-center font-bold focus:border-[#ff9100] focus:ring-1 focus:ring-[#ff9100] outline-none text-sm transition-all disabled:cursor-not-allowed"
                                            placeholder="Freq"
                                        />
                                    </div>
                            ...
                            <div className="p-4 bg-black/10">
                                ...
                                >
                                    <Plus className="w-5 h-5" /> Add New Error Proof Definition
                                </button>
                            </div>
...
export default ConfigErrorProof;
