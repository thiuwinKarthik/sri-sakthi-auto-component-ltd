  ...
  return (
    <div className="relative w-full">
      ...
      <input
        ...
        className="w-full p-3 text-sm font-medium border-2 border-gray-300 rounded-lg bg-white text-gray-900 focus:border-orange-500 outline-none placeholder-gray-500 shadow-sm"
...
const DisaMachineCheckList = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Logic: Clear Checklist if Date Doesn't Match
      ...
      const processedChecklist = res.data.checklist.map(item => {
        ...
        return {
          ...item,
          IsDone: isSameDate ? item.IsDone : false 
        };
      });
  ...
  // --- FIX: TOGGLE LOGIC FOR OK BOXES ---
  const handleOkClick = (item) => {
    setChecklist(prev => prev.map(c => 
      // If IDs match, flip the IsDone value (True -> False -> True)
      c.ChecklistId === item.ChecklistId ? { ...c, IsDone: !c.IsDone } : c
    ));
  };
  ...
  const handleNotOkClick = (item) => {
    if (!headerData.operatorName) {
      ...
      return alert("⚠️ Please select your name at the bottom to sign the report.");
    }
  ...
  const submitReport = async () => {
    if (!ncForm.ncDetails || !ncForm.responsibility) {
      alert("Please fill in Details and Responsibility.");
      return;
    }
    try {
      ...
      // Update Reports Map so UI updates immediately
      setReportsMap(prev => ({
        ...prev,
      ...
      // Ensure item is unchecked (IsDone = false) in list
      setChecklist(prev => prev.map(c => 
  ...
  const handleBatchSubmit = async () => {
    if (!headerData.operatorName) {
      return alert("⚠️ Please select your name (Operator) to sign the checklist.");
    }
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10 px-4 flex justify-center pb-24">
      <div className="w-full max-w-6xl bg-white shadow-xl rounded-2xl overflow-visible relative flex flex-col">
        ...
        {/* Footer with Operator Selection & Submit */}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden animate-fade-in-up">
            ...
            <div className="bg-red-600 p-5 flex justify-between items-center">
              ...
              <button onClick={() => setIsModalOpen(false)} className="text-white hover:bg-red-700 rounded-full p-1 text-xl">&times;</button>
            </div>
            ...
            <div className="p-8 space-y-6 max-h-[75vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="col-span-2">
                  <label className="text-xs font-bold text-gray-500 uppercase block mb-1">Non-Conformity Details</label>
                  <textarea 
                    rows="2"
              ...
              <div className="pt-4 border-t border-gray-100">
                <button 
                  ...
                  className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-4 rounded-lg uppercase tracking-wider shadow-lg"
                >
                  Save & Log Report
                </button>
              </div>
